...
public class Task {
	public static void start(Container cont) {
		if(cont == null) {
			throw new NullPointerException("line is null");
		}
		StringBuilder strB;
		String[] str = cont.toArray();
		for(int i=0;i<cont.size();i++)
		{
			strB = new StringBuilder(str[i]);
			clottingSpace(strB);
			ArrayList<StringBuilder> line = findArray(strB); 
		task(line);
		}
		cont.setContainer(str);
	}
